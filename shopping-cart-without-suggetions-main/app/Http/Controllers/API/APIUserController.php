...
class APIUserController extends Controller
{
    ...
    public function login(Request $request)
    {
        try {
            ...
            $user = User::getData(true)->where('usertype', 3)->where('email', $request->email)->first();
            if ($user) {
                if (Hash::check($request->password, $user->password)) {
                    return $this->successResponse(code: 200, data: ['user' => $user, 'stores' => Shop::getData(true)->get()]);
                } else {
