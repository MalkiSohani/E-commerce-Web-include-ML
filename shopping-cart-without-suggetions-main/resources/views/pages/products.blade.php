    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        ...
        <div class="container-fluid py-4">
            <div class="row">
                                    ...
                                    <tbody>
                                        @forelse ($products as $product)
                                            <tr>
                                                ...
                                                <td class="text-xs text-secondary mb-0">{{ $product->name }}<p class="text-success text-xs mt-2">{{ $product->rfid }}</p></td>
                                                <td class="text-center text-xs text-secondary mb-0">{{ $product->qty }}</td>
                                                <td class="text-xs text-secondary mb-0">{{ format_currency($product->price) }}</td>
                                                <td class="text-xs text-secondary mb-0 text-center">
                                                    <strong>{{ $product->ratings[0]->rate ?? 0 }}</strong> <small>/ 5</small>
                                                </td>
                                                <td class="text-xs text-secondary mb-0 text-left"><span
                                                        class="badge badge-sm bg-gradient-{{ (new App\Models\Colors())->getColor($product['status']) }}">{{ App\Models\Shop::$status[$product['status']] }}</span>
                                                </td>
                ...
                <div class="col-md-4">
                    <form autocomplete="off" action="{{ route('admin.products.enroll') }}" enctype="multipart/form-data"
                                ...
                                <div class="card-body pt-0">
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="row ">
                                                <div class="col-md-12">
                                                    <label for="shop"><small class="text-dark">
                                                            Shop</small></label>
                                            ...
                                            <div class="row mt-1">
                                                <div class="col-md-12">
                                                    <label for="qty"><small class="text-dark">
                                                            Quantity{!! required_mark() !!}</small></label>
                                            ...
                                            <div class="row mt-1">
                                                <div class="col-md-12">
                                                    <label for="is_fridge_item"><small class="text-dark">
                                                            Is Fridge Item</small></label>
                                                    <select name="is_fridge_item" id="is_fridge_item"
                                                        class="form-control">
                                                        <option value="1">Yes</option>
                                                        <option value="2">No</option>
                                                    </select>
                                                    @error('is_fridge_item')
                                                        <span class="text-danger"><small>{{ $message }}</small></span>
                                                    @enderror
                                                </div>
                                            </div>
            ...
            showAlert('Are you sure to edit this record ?', function() {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#is_fridge_item').val(response.is_fridge_item);
                    }
                });
            });
        }
